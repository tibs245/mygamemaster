...
CMD ["sh", "-c", "\
  ...
  cd /app/modules/gaming/mj-tonnerre/scripts && python3 -m unittest discover -s tests && \
  echo '=== Suite 2 — runtime hooks ===' && \
  cd /app/modules/gaming/mj-tonnerre/hooks && python3 test_hooks.py && \
  echo '=== Suite 3 — TTS pipeline ===' && \
  cd /app/modules/gaming/mj-tonnerre-tts/tests && python3 test_tts.py && \
