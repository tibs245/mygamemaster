...
| `verbosity` | rendered detail | **Persisted** block of the Steward + level `meta.verbosity` | none |
...
|---|---|---|
| **soft** | `images`, `verbosity`, `tracabilite`, `tts` | **safe anytime** — no effect on game state (rendering / internal logs only) |
...
import _lib as L
monde = L.load_json("data/mj-tonnerre/campaigns/la-naissance-dun-roi/world.json") or {}
...
- **Hot vs. cold** (§3): `world.json` is reloaded each turn → changes **hot** (next
  ...
  (`images`, `verbosity`, `tracabilite`, `tts`): safe to toggle anytime; **structural**
