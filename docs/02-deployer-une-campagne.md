...
4. **Seed**: on the very first deploy, copies `data/mj-tonnerre/campaigns/<data_dir>/` into the
   data volume.
...
1. Create the game data folder:
   ```bash
   cp -r data/mj-tonnerre/campaigns/_template data/mj-tonnerre/campaigns/<your-data-dir>
   ```
   Then fill in `world.json` (the `mj-tonnerre-initiation` skill drives a questionnaire that helps
