...
|---|---|---|---|
| `hermes-<id>-data` | `/opt/data/mygamemaster/campaigns` | **Game data**: `world.json`, `npcs.json`, `characters/`, `sessions/`, `events.json`, `geo.json`, `images/`, `collecte.csv`, **`.banquier/`** (ledger + snapshots) | volume export → `tar.gz` |
...
> **Initial data** remains versioned in `data/mygamemaster/campaigns/<data_dir>/` (the seed).
