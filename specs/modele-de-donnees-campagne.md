...
A campaign lives in `data/mygamemaster/campaigns/<slug>/`. During deployment, this folder is copied
...
| `system` | mechanics: `resolution`, `combat`, `competences`, `sante`, `niveaux_difficulte`, `nat1`/`nat20`, `crunch`, `construction_royaume` |
...
> **Thematic modules** are loaded conditionally by the `mygamemaster` skill based on
> `modules.<x>.actif`. References for these modules are in
> `modules/gaming/mygamemaster/references/modules/`.

## `npcs.json` — Element (array)

Keys per NPC: `nom`, `titre`, `description`, `attitude`, `relation_niveau`, `localisation_actuelle`,
`stats`, `modificateurs`, `competences_observees`, `inventaire`, `established_facts`,
...
the `close_session.py` script in the `mygamemaster` skill.

## `base_items.yaml` — Shared Items (at `data/mygamemaster/` level)
...
`modules/gaming/mygamemaster/scripts/`) and schemas under `scripts/schemas/`. The `smoke-test.yml`
