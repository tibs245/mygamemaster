...
- **Pivot variable**: `campaign_slug` (game slug), derived from the `slug` field in `games.yml`.
...
- **Instance dimension**: `instance` (default `main`) allows deploying the **same campaign
  ...
  unchanged). The seed always comes from `campaign_data_dir` → each instance has its own
...
The role variable names (`campaign_slug`, `campaign_title`, etc.) are the English identifiers
used throughout the role (renamed from the former French `campagne_*` names).

### `group_vars/all/main.yml` (excerpt)

```yaml
hermes_image_name: mygamemaster
hermes_image_tag: latest
hermes_base_image: docker.io/nikolaik/python-nodejs:python3.11-nodejs20
container_data_root: /opt/data/mygamemaster/campaigns   # inside the container
...
default_language: fr
instance: main
deploy_id: "{{ campaign_slug if (instance | string) in ['main', ''] else (campaign_slug ~ '-' ~ instance) }}"
```
...
|---|---|---|
| `config` | renders `config.yaml` + `SOUL.md`, notifies `restart_campaign` | `update-config.yml` |
| `credentials` | resolves the token (vault) → writes the `.env` at `0600`, notifies `restart_campaign` | `update-credentials.yml` |
...
2. Render `config.yaml` and `SOUL.md` into `~/.config/mygamemaster/rendered/<deploy_id>/` on the
   deployment host.
...
4. **Initial data seed**: if the data volume is empty, copy
   `data/mygamemaster/campaigns/<campaign_data_dir>/` into it (first deployment only).
...
### Quadlet (`hermes-campaign.container.j2`) — key sections
...
`~/.config/mygamemaster/<deploy_id>.env` on the deployment host and is **never** committed.
