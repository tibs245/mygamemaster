...
```
CAMP=.hermes/mygamemaster/campaigns/la-naissance-dun-roi
...
- `--json`: machine output. `--log`: log file (default `scripts/jets.log` or `$MGM_ROLL_LOG`). `--no-log`: do not log.
...
`events.json`, `characters/*`, `sessions/*`, etc.) and validates their syntax.
...
- `pre-commit.hook`: hook template (POSIX sh). Contains the placeholder
  `__MGM_SCRIPTS_DIR__` replaced at install with the absolute path of this folder.
...
- Emergency bypass: `MGM_SKIP_HOOK=1 git commit …`.
- Override script path: `MGM_TONNERRE_SCRIPTS=/path git commit …`.
...
Gaps detected:
- location in `lieux_visites[]` missing from `universe.regions[].lieux` → **blocking**;
...
`events.json`; otherwise → max "Day N" mentioned in timeline + sessions.
...
```sh
CAMP=.hermes/mygamemaster/campaigns/<campaign>
SCRIPTS=/opt/modules/gaming/mygamemaster/scripts
...
- **UT** mode → current time = last `t` from `events.json` (in UT);
...
Provided schemas:
- `world.schema.json` — incl. `modules` block, `global_state.factions` with
...
- `npcs.schema.json` — canonical format (required `established_facts`/`hypotheses_mj`;
  tolerates bare list or `{"pnj":[...]}`);
- `character.schema.json` — `meta.nom_perso`, `stats`, `inventaire`, `sante`;
...
their sheet (`characters/<discord_id>.json`). Preferences persist across
...
## 13. `emotions.py` — Character Emotions (skill `mygamemaster-emotions`)
...
itself: `npcs.json` (NPCs, canonical) or `characters/*.json` (opt-in PCs).
