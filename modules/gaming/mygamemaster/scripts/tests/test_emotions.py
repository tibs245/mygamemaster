#!/usr/bin/env python3
"""
test_emotions.py — Tests for the character emotions module (mygamemaster-emotions).
...
Covers:
  ...
  * CLI: init/apply/adjust/decay/get/summary against a throwaway campaign,
    auto-init on apply, PC sheet opt-in (characters/*.json), exit codes,
...
class TestUpdateRules(unittest.TestCase):
    ...
    def test_betrayal_moves_emotions_in_the_logical_direction(self):
        ...
        self.assertLess(new["trust"], state["trust"])   # trust collapses
        self.assertGreater(new["anger"], state["anger"])      # anger rises
        self.assertGreater(new["fear"], state["fear"])          # fear rises
        self.assertGreater(new["surprise"], state["surprise"])  # shock
        self.assertIn("anger", applied)

    def test_kindness_repaid_raises_warmth(self):
        new, _ = E.apply_event(neutral_state(), "kindness")
        self.assertGreater(new["trust"], E.DEFAULT_TEMPERAMENT["trust"])
        self.assertGreater(new["joy"], E.DEFAULT_TEMPERAMENT["joy"])

    def test_exact_delta_applied(self):
        state = neutral_state()  # trust 0.3
        new, applied = E.apply_event(state, "promise_kept")  # trust +0.25
        self.assertAlmostEqual(new["trust"], 0.55, places=3)
        self.assertAlmostEqual(applied["trust"], 0.25, places=3)
    ...
    def test_intensity_scales_deltas(self):
        ...
        gap_full = full["fear"] - E.DEFAULT_TEMPERAMENT["fear"]
        gap_half = half["fear"] - E.DEFAULT_TEMPERAMENT["fear"]
        self.assertAlmostEqual(gap_half, gap_full / 2, places=3)

    def test_clamped_to_unit_interval(self):
        state = dict(neutral_state(), fear=0.9, trust=0.05)
        new, applied = E.apply_event(state, "attack")  # fear +0.4, trust -0.3
        self.assertEqual(new["fear"], 1.0)
        self.assertEqual(new["trust"], 0.0)
        # `applied` records the EFFECTIVE (post-clamping) deltas.
        self.assertAlmostEqual(applied["fear"], 0.1, places=3)
        self.assertAlmostEqual(applied["trust"], -0.05, places=3)
    ...
    def test_apply_deltas_ignores_garbage(self):
        new, applied = E.apply_deltas(neutral_state(),
                                      {"fear": "huge", "inconnu": 0.4, "joy": True})
...
class TestDecay(unittest.TestCase):
    ...
    def test_decay_closes_half_the_gap(self):
        temperament = neutral_state()                       # fear 0.2
        state = dict(temperament, fear=0.8)
        new = E.decay_state(state, temperament, rate=0.5)
        self.assertAlmostEqual(new["fear"], 0.5, places=3)  # 0.8 → halfway to 0.2

    def test_decay_works_upward_too(self):
        temperament = dict(neutral_state(), joy=0.6)
        state = dict(temperament, joy=0.0)                 # grief-stricken optimist
        new = E.decay_state(state, temperament, rate=0.5)
        self.assertAlmostEqual(new["joy"], 0.3, places=3)  # drifts back up

    def test_full_rate_lands_on_temperament(self):
        temperament = neutral_state()
        state = dict(temperament, anger=1.0, joy=0.0)
    ...
    def test_surprise_decays_fast_even_at_low_rate(self):
        ...
        state = dict(temperament, surprise=1.0, fear=1.0)
        new = E.decay_state(state, temperament, rate=0.1)
        self.assertLessEqual(new["surprise"], 0.2)          # ≥ 0.8 of gap closed
        self.assertAlmostEqual(new["fear"], 0.92, places=3)  # others at rate 0.1

    def test_bad_rate_falls_back(self):
        temperament = neutral_state()
        state = dict(temperament, fear=0.8)
        new = E.decay_state(state, temperament, rate="not-a-number")
        self.assertAlmostEqual(new["fear"], 0.5, places=3)  # default 0.5


class TestSummaryAndFailOpen(unittest.TestCase):

    def test_no_emotions_means_no_output(self):
        self.assertEqual(E.summary_line({"name": "Berthe"}), "")
        self.assertEqual(E.summary_line({"name": "Berthe", "emotions": "high"}), "")
        self.assertEqual(E.summary_line(None), "")
        self.assertEqual(E.summary_block([{"name": "A"}, {"name": "B"}]), "")

    def test_summary_shows_deviation_and_reason(self):
        fiche = {"name": "Berthe", "emotions": {
            "etat": dict(neutral_state(), fear=0.7, trust=0.1),
            "temperament": neutral_state(),
            "history": [{"event": "betrayal", "deltas": {},
    ...
    def test_block_caps_and_orders_by_recency(self):
        fiches = []
        for i in range(8):
            fiches.append({"name": "P%d" % i, "emotions": {
                "etat": dict(neutral_state(), fear=0.6),
                "temperament": neutral_state(),
                "history": [{"event": "threat", "deltas": {},
                                "reason": "r", "session": i}],
            }})
    ...
    def test_normalize_state_tolerates_garbage(self):
        out = E.normalize_state({"fear": "deep", "joy": 2.4, "anger": -1})
        self.assertEqual(out["fear"], E.DEFAULT_TEMPERAMENT["fear"])
        self.assertEqual(out["joy"], 1.0)
        self.assertEqual(out["anger"], 0.0)
...
class TestHistory(unittest.TestCase):
    ...
    def test_record_appends_reason_and_session(self):
        emo = {"history": []}
        E.record_history(emo, "threat", {"fear": 0.3}, "drew a blade", session=4)
        entry = emo["history"][-1]
        self.assertEqual(entry["event"], "threat")
        self.assertEqual(entry["reason"], "drew a blade")
        self.assertEqual(entry["session"], 4)
        self.assertEqual(entry["deltas"], {"fear": 0.3})

    def test_history_is_capped(self):
        emo = {"history": []}
        for i in range(E.HISTORY_MAX + 7):
            E.record_history(emo, "kindness", {"joy": 0.1}, "r%d" % i)
        self.assertEqual(len(emo["history"]), E.HISTORY_MAX)
        self.assertEqual(emo["history"][-1]["reason"],
...
class TestCli(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._write("world.json", {"meta": {"name": "Test"}})
        self._write("npcs.json", [
            {"name": "Berthe", "established_facts": [], "gm_hypotheses": []},
            {"name": "Firmin", "established_facts": [], "gm_hypotheses": []},
        ])
        self._write("characters/403.json", {
    ...
    def _pnj(self, name):
        data = json.loads((self.camp / "npcs.json").read_text(encoding="utf-8"))
        return next(f for f in data if f["name"] == name)

    def test_full_lifecycle(self):
        rc, out, _ = run_cli("init", self.camp, "Berthe", "fear=0.4")
        self.assertEqual(rc, 0)
        rc, out, _ = run_cli("apply", self.camp, "Berthe", "--event", "betrayal",
                             "--reason", "sold the players out", "--session", "2")
        self.assertEqual(rc, 0, out)
        emo = self._pnj("Berthe")["emotions"]
        self.assertLess(emo["etat"]["trust"], 0.3)
        self.assertEqual(emo["history"][-1]["session"], 2)
        self.assertEqual(emo["history"][-1]["reason"], "sold the players out")
        ...
        self.assertEqual(rc, 0)
        self.assertIn("history", json.loads(out) or {})
    ...
    def test_init_twice_requires_force(self):
        ...
        rc, _, _ = run_cli("init", self.camp, "Berthe", "joy=0.9", "--force")
        self.assertEqual(rc, 0)
        self.assertEqual(self._pnj("Berthe")["emotions"]["temperament"]["joy"], 0.9)
    ...
    def test_apply_auto_initializes(self):
        ...
        self.assertGreater(emo["etat"]["joy"], E.DEFAULT_TEMPERAMENT["joy"])

    def test_adjust_requires_reason(self):
        rc, _, _ = run_cli("adjust", self.camp, "Berthe", "fear=+0.2")
        self.assertEqual(rc, 2)                    # argparse: --reason required
        rc, _, _ = run_cli("adjust", self.camp, "Berthe", "fear=+0.2",
                           "--reason", "the mist reached the door")
        self.assertEqual(rc, 0)
        self.assertEqual(self._pnj("Berthe")["emotions"]["history"][-1]["reason"],
    ...
    def test_pc_sheet_opt_in(self):
        ...
        self.assertEqual(rc, 0)
        fiche = json.loads((self.camp / "characters" / "403.json")
                           .read_text(encoding="utf-8"))
        self.assertGreater(fiche["emotions"]["etat"]["joy"],
                           E.DEFAULT_TEMPERAMENT["joy"])
        # PC emotions are NEVER injected: summary only reads npcs.json.
    ...
    def test_summary_fail_open(self):
        ...
        # Broken npcs.json → still code 0, still silent.
        (self.camp / "npcs.json").write_text("{not json", encoding="utf-8")
        rc, out, _ = run_cli("summary", self.camp)
        self.assertEqual(rc, 0)
        self.assertEqual(out.strip(), "")
