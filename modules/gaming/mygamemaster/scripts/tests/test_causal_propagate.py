...
MANDATORY cases (contract §12):
  ...
  * all emitted events validate scheduled_event.schema.json;
...
class TestPropagation(unittest.TestCase):
    ...
    # — Schema: all derivatives validate scheduled_event.schema.json -------
...
class TestAppliquer(unittest.TestCase):
    ...
    def test_cree_le_fichier_avec_meta(self):
        ...
        self.assertIn("NEVER merge", data["meta"]["note"])
    ...
    def test_append_preserve_existant(self):
        ...
        prealable = {
            "meta": {"campagne": "Fixture", "version": 1, "note": "NEVER merge"},
            "events": [{
    ...
    def test_n_ecrit_jamais_evenements_json(self):
        ...
        (self.camp / "events.json").write_text(
            json.dumps({"events": [{"id": "ORIGINAL"}]}, ensure_ascii=False),
            encoding="utf-8")
