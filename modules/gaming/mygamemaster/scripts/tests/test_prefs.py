...
def _sheet(nom_joueur, prefs=None):
    fiche = {
        "meta": {"player_name": nom_joueur, "discord_id": "x", "character_name": nom_joueur},
        "personal_notes": {"goals": [], "relations": {}, "secrets": []},
    }
...
class PrefsTest(unittest.TestCase):
    ...
    def _sheet_json(self, discord_id):
        return json.loads((self.camp / "characters" / f"{discord_id}.json").read_text())
    ...
    def test_set_documented_json_bool(self):
        code, _ = self._run(BOB, "set", "enjoys_deception", "false")
        self.assertEqual(code, 0)
        self.assertIs(self._sheet_json(BOB)["preferences"]["enjoys_deception"], False)

    def test_set_documented_json_list(self):
        code, _ = self._run(BOB, "set", "tone_likes", '["tension","action"]')
        self.assertEqual(code, 0)
        self.assertEqual(self._sheet_json(BOB)["preferences"]["tone_likes"],
                         ["tension", "action"])
    ...
    def test_set_preserves_other_sheet_fields(self):
        self._run(ALICE, "set", "pacing", "fast")
        fiche = self._sheet_json(ALICE)
        self.assertIn("personal_notes", fiche)
        self.assertEqual(fiche["meta"]["player_name"], "Alice")

    # ── UNSET ─────────────────────────────────────────────────────────────
    def test_unset_documented_key(self):
        code, _ = self._run(ALICE, "unset", "pacing")
        self.assertEqual(code, 0)
        self.assertNotIn("pacing", self._sheet_json(ALICE)["preferences"])
    ...
    def test_dry_run_writes_nothing(self):
        before = (self.camp / "characters" / f"{ALICE}.json").read_text()
        code, _ = self._run(ALICE, "set", "pacing", "changed", "--dry-run")
        self.assertEqual(code, 0)
        after = (self.camp / "characters" / f"{ALICE}.json").read_text()
        self.assertEqual(before, after)

    # ── COMPARTMENTALIZATION ──────────────────────────────────────────────
    def test_set_touches_only_named_player(self):
        bob_before = (self.camp / "characters" / f"{BOB}.json").read_text()
        self._run(ALICE, "set", "pacing", "fast")
        bob_after = (self.camp / "characters" / f"{BOB}.json").read_text()
    ...
    def test_set_without_value_exit_1(self):
        code, _ = self._run(ALICE, "set", "pacing")
        self.assertEqual(code, 1)

    def test_json_output(self):
        code, out = self._run(ALICE, "set", "pacing", "fast", "--json")
        self.assertEqual(code, 0)
        payload = json.loads(out)
        self.assertEqual(payload["preferences"]["pacing"], "fast")
