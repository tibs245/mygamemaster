...
or from the repository root:
    python3 -m unittest modules.gaming.mygamemaster.scripts.tests.test_scene_brief -v
...
class TestAxeRelationnel(unittest.TestCase):
    ...
    def test_enjeux_dedupliques(self):
        cles = [(e["actor"], e["type"], e.get("_vers")) for e in self.brief["enjeux"]]
            ...
            "actors": [
                {"id": "acteur:garde", "name": "The Guard", "type": "npcs",
                 ...
                 "relations": [{"vers": "acteur:cendre", "type": "serment",
                                "intensite": 0.7}]},
                {"id": "acteur:oscar", "name": "Oscar", "type": "npcs", "majeur": True,
                 "trajectory": [{"lieu": "lieu:scene", "de": 0, "a": None}],
                 "relations": []},
                # Cendre IN MOVEMENT toward the scene: would cross the location, but is a
                # PC → must NEVER appear in MOUVEMENT.
                {"id": "acteur:cendre", "name": "Cendre", "type": "npcs", "majeur": True,
                 "trajectory": [
...
class TestAxeTemporel(unittest.TestCase):
    ...
    def setUp(self):
        ...
        progs = {
            "meta": {"campagne": "x", "version": 1, "note": "NEVER merge"},
            "events": [
...
class TestMouvementCroisement(unittest.TestCase):
    ...
    def test_mouvement_detecte_le_croisement(self):
        ...
        m = next(m for m in b["mouvement"] if m["actor"] == "acteur:firmin")
        for clef in ("actor", "T", "lieu", "narratif"):
    ...
    def test_acteur_statique_n_apparait_pas_en_mouvement(self):
        ...
        b = SB.scene_brief(self.camp, LIEU_CABANE, T=1200, fenetre_ut=432)
        self.assertNotIn("acteur:berthe", {m["actor"] for m in b["mouvement"]})
