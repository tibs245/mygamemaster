...
or directly by file path:
    python3 modules/gaming/mygamemaster/scripts/tests/test_worldlib.py
...
# top-level module (the `mygamemaster` folder contains a dash: it is NOT a
...
def _acteurs_jouet() -> dict:
    return {
        ...
        "actors": [
            {
                "id": "acteur:alpha", "name": "Alpha", "type": "npcs",
                "lod": "tiede", "majeur": True, "but_long_terme": "—",
                "situation": "—", "ressources": {},
                "trajectory": [{"lieu": "A", "de": 0, "a": None}],
                "plan": [],
                "relations": [
            ...
            {
                "id": "acteur:beta", "name": "Beta", "type": "npcs",
                "lod": "froid", "majeur": True, "but_long_terme": "—",
                "situation": "—", "ressources": {},
                # No 'trajectory': only localisation_id (backward-compat).
                "localisation_id": "B",
                "plan": [],
                "relations": [
...
class TestPjIds(unittest.TestCase):
    """GENERIC PC resolution (cascade meta.pj_ids > meta.pj_id > MGM_PJ_ID > [])."""

    def setUp(self):
        # Isolate MGM_PJ_ID: save/restore to avoid polluting other tests.
        self._env_sauve = os.environ.pop("MGM_PJ_ID", None)

    def tearDown(self):
        if self._env_sauve is None:
            os.environ.pop("MGM_PJ_ID", None)
        else:
            os.environ["MGM_PJ_ID"] = self._env_sauve
    ...
    def test_pj_ids_depuis_env_splitte_virgules(self):
        # No PC meta → env MGM_PJ_ID split on commas, spaces stripped.
        os.environ["MGM_PJ_ID"] = " acteur:a , acteur:b ,"
    ...
    def test_pj_ids_meta_prime_sur_env(self):
        ...
        os.environ["MGM_PJ_ID"] = "acteur:env"
...
class TestJsonSur(unittest.TestCase):
    ...
    def test_chemin_campagne_absolu(self):
        res = W.chemin_campagne("data/mygamemaster/campaigns/la-naissance-dun-roi")
...
class TestMDS(unittest.TestCase):
    ...
    def test_stress_normalise_matrice_reelle_sous_seuil(self):
        ...
        self.assertTrue(dep, "regles.temps.deplacements not found in world.json")
...
class TestActeursRelations(unittest.TestCase):
    ...
    def test_trajectoire_de_retrocompat_localisation(self):
        # beta has no 'trajectory' but has a 'localisation_id' → synthetic stay.
        idx = W.index_acteurs(self.acteurs)
