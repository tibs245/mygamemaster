...
def _pnj_charger(campagne: Path):
    """Load npcs.json → (raw_data, list). Tolerates bare list OR {"npcs":[...]}."""
...
def cmd_extract(args) -> int:
    ...
    if args.faction is not None:
        ...
        cible_type, cible_nom = "npcs", args.pnj
...
def cmd_reintegrate(args) -> int:
    ...
    nom = slice_obj["name"]
    if cible_type not in ("faction", "npcs"):
...
def _reintegrate_faction(args, campagne, monde_path, monde, slice_obj) -> int:
    ...
    idx = _index_par_nom(factions, nom)
    if idx < 0:
        print(f"❌ Faction «{nom}» has disappeared from world.json.", file=sys.stderr)
        return 1
