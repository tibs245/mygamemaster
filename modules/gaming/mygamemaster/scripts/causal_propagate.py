...
def programmer_evenement(*, cible: str, T: int, type: str,
    ...
    """Builds a SCHEDULED event conforming to scheduled_event.schema.json (§8.3).
...
def appliquer(campagne: Path, evenements: list[dict]) -> int:
    ...
    if not isinstance(fichier, dict) or "events" not in fichier:
        fichier = {"meta": _meta_par_defaut(campagne), "events": []}
    if not isinstance(fichier.get("events"), list):
        fichier["events"] = []

    existants = {e.get("id") for e in fichier["events"]
...
def _charger_evt_arg(spec: str) -> dict | None:
    ...
    Accepts either a standalone event or {'events':[…]} (takes the 1st).
    """
    if spec is None:
        return None
    spec = str(spec)
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "Writes ONLY evenements_programmes.json (never events.json)."
        ),
    )
