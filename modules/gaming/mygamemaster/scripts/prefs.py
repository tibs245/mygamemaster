...
(`characters/<discord_id>.json`). They persist across sessions and are
...
Documented keys (all optional; a missing/empty block changes nothing — fail-open):
  pacing            string  — preferred pacing
  tone_likes          list    — tone likes
  tone_dislikes         list    — tone dislikes
  combat_verbosity  string  — combat verbosity (e.g. concise|vivid)
  spotlight         string  — spotlight preferences
  content_boundaries   list    — content boundaries
  enjoys_deception  bool    — enjoys being deceived (fair foreshadowing)
...
Usage:
  ...
  python3 prefs.py <campaign> <discord_id> set pacing "slow-burn investigation"
  python3 prefs.py <campaign> <discord_id> set enjoys_deception true
  python3 prefs.py <campaign> <discord_id> set tone_likes '["mystery","lore"]'
  ...
  # Remove a key:
  python3 prefs.py <campaign> <discord_id> unset pacing
...
def _resoudre_fiche(campagne: str, discord_id: str) -> Path:
    """Path to characters/<discord_id>.json (or direct path to a .json)."""
    base = _resoudre(campagne)
    if base.suffix == ".json":
        return base
    return base / "characters" / f"{discord_id}.json"
...
def main(argv=None) -> int:
    ...
    ap.add_argument("campagne",
                    help="campaign directory, OR direct path to characters/<id>.json")
