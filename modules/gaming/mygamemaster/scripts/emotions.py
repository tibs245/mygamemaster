...
Purpose (skill `mygamemaster-emotions`): give the GM a compact, legible and
...
small (no over-engineering):
  ...
  * 6 core emotions, intensities 0..1 — `joy`, `trust`, `fear`, `anger`,
    `sadness`, `surprise`.
    Plutchik-inspired palette: `trust` is kept because trust drives most
...
`npcs.json` for NPCs (canonical), `characters/*.json` for opt-in PCs.
...
EMOTIONS = ("joy", "trust", "fear", "anger", "sadness", "surprise")
...
EVENT_RULES = {
    "betrayal":             {"trust": -0.40, "anger": +0.30, "fear": +0.15,
                             "sadness": +0.20, "surprise": +0.30},
    "deception_discovered": {"trust": -0.35, "anger": +0.25, "surprise": +0.25},
    "promise_broken":       {"trust": -0.30, "anger": +0.20, "sadness": +0.10},
    "promise_kept":         {"trust": +0.25, "joy": +0.10},
    "kindness":             {"trust": +0.15, "joy": +0.10},
    "gift":                 {"joy": +0.15, "trust": +0.10, "surprise": +0.10},
    "rescue":               {"trust": +0.30, "joy": +0.20, "fear": -0.25},
    "threat":               {"fear": +0.30, "anger": +0.15, "trust": -0.15},
    "attack":               {"fear": +0.40, "anger": +0.35, "trust": -0.30,
                             "surprise": +0.20},
    "insult":               {"anger": +0.25, "trust": -0.10, "joy": -0.10},
    "comfort":              {"sadness": -0.20, "fear": -0.15, "trust": +0.15},
    "loss":                 {"sadness": +0.40, "joy": -0.25, "surprise": +0.10},
    "reunion":              {"joy": +0.25, "sadness": -0.20, "surprise": +0.15},
    "victory":              {"joy": +0.30, "trust": +0.10, "fear": -0.20},
    "good_news":            {"joy": +0.20, "surprise": +0.15},
    "bad_news":             {"sadness": +0.20, "fear": +0.15, "surprise": +0.15},
}
...
TONE_LABELS = {
    "joy": "cheerful", "trust": "trusting", "fear": "fearful",
    "anger": "angry", "sadness": "sorrowful", "surprise": "startled",
}
...
def _last_shift(emo_obj: dict) -> str:
    ...
    hist = emo_obj.get("history")
    if not isinstance(hist, list) or not hist:
        return ""
    last = hist[-1]
    if not isinstance(last, dict):
        return ""
    reason = str(last.get("reason") or last.get("event") or "").strip()
    if not reason:
        return ""
...
def summary_line(fiche: dict) -> str:
    ...
    name = (fiche.get("name")
            or (fiche.get("meta") or {}).get("character_name")
            or "?")
...
def summary_block(fiches: list, max_chars: int | None = None,
    ...
    for fiche in fiches or []:
        ...
        hist = emo.get("history") if isinstance(emo.get("history"), list) else []
        last_sess = -1
        for entry in hist:
...
# ════════════════════════════════════════════════════════════════════════════
#  Sheet location & persistence (NPCs in npcs.json, opt-in PCs in characters/)
# ════════════════════════════════════════════════════════════════════════════
...
def _pnj_list(data) -> list:
    """Normalises npcs.json into a list of sheets (bare list or {'npcs': [...]})."""
    if isinstance(data, list):
        return [x for x in data if isinstance(x, dict)]
    if isinstance(data, dict) and isinstance(data.get("npcs"), list):
        return [x for x in data["npcs"] if isinstance(x, dict)]
    return []
...
class CharacterRef:
    ...
    def __init__(self, path: Path, container, fiche: dict, kind: str):
        ...
        self.kind = kind          # 'npcs' | 'character'

    @property
    def name(self) -> str:
        return (self.fiche.get("name")
                or (self.fiche.get("meta") or {}).get("character_name") or "?")
...
def find_character(camp: Path, name: str) -> CharacterRef | None:
    """Locates `name`: npcs.json first (canonical home of NPC emotions), then
    opt-in PC sheets in characters/ (matched on meta.nom_perso).
    ...
    pnj_path = camp / "npcs.json"
    pnj_doc = _load_json(pnj_path)
    for fiche in _pnj_list(pnj_doc):
        candidates.append({"_nom_recherche": str(fiche.get("name", "")),
                           "_ref": (pnj_path, pnj_doc, fiche, "npcs")})
    ...
    if pdir.is_dir():
        for p in sorted(pdir.glob("*.json")):
            ...
            if not isinstance(fiche, dict):
                continue
            nom = (fiche.get("meta") or {}).get("character_name") or fiche.get("name")
            if nom:
                candidates.append({"_nom_recherche": str(nom),
                                   "_ref": (p, fiche, fiche, "character")})

    if not candidates:
        _err("❌ no character sheet found in %s (npcs.json / characters/)" % camp)
        return None
...
def ensure_emotions(fiche: dict, temperament: dict | None = None) -> dict:
    ...
    if not isinstance(emo.get("history"), list):
        emo["history"] = []
    return emo
...
def record_history(emo: dict, event: str, deltas: dict, reason: str,
    ...
    entry = {"event": str(event), "deltas": dict(deltas or {}),
             "reason": str(reason or event)}
    if session not in (None, ""):
        entry["session"] = session
    entry["ts"] = datetime.now().isoformat(timespec="seconds")
    hist = emo.setdefault("history", [])
    hist.append(entry)
...
def _parse_pairs(pairs: list[str], signed: bool):
    """['fear=0.4', 'trust=-0.2'] → dict. None + message on bad syntax.
...
def cmd_get(args) -> int:
    ...
    if camp is None:
        return 2
    ref = find_character(camp, args.name)
    if ref is None:
        return 1
...
def cmd_decay(args) -> int:
    ...
    if camp is None:
        return 2
    if args.name:
        refs = [find_character(camp, args.name)]
        if refs[0] is None:
            return 1
    else:
        ...
        if not sheets:
            ...
            return 0
        refs = [CharacterRef(pnj_path, pnj_doc, f, "npcs") for f in sheets]
...
def cmd_summary(args) -> int:
    ...
    try:
        ...
        fiches = _pnj_list(_load_json(Path(camp) / "npcs.json"))
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        ...
        description=("Character emotional state (6 emotions 0..1 + temperament): "
                     ...
                     "history. NPCs in npcs.json, opt-in PCs in characters/."),
    )
