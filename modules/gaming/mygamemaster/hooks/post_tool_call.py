...
def compute_deltas(kind, b, a, rel):
    ...
    if kind == "session":
        ...
        nom = a.get("name") or b.get("name") or "PJ"
        if "inventory" in a and "inventory" in b and a["inventory"] != b["inventory"]:
            d("🎒", "Inventory of %s: %s → %s item(s)" % (nom, b["inventory"], a["inventory"]), "inventory")
        elif "inventory" in a and "inventory" not in b:
            d("🎒", "Inventory of %s modified (%s item(s))" % (nom, a["inventory"]), "inventory")
        if "pv" in a and "pv" in b and a["pv"] != b["pv"]:
            d("❤️", "%s — PV : %s → %s" % (nom, b["pv"], a["pv"]), "health")
        if "equipment" in a and "equipment" in b and a["equipment"] != b["equipment"]:
            d("⚔️", "Equipment of %s: %s → %s" % (nom, b["equipment"], a["equipment"]), "inventory")
        ...
        if a.get("day") is not None and a.get("day") != b.get("day"):
            d("🕒", "Day: %s → %s" % (b.get("day"), a.get("day")), "time")
        if a.get("hour") is not None and a.get("hour") != b.get("hour"):
            d("🕒", "Time: %s → %s" % (b.get("hour"), a.get("hour")), "time")

    # Fallback: change detected but untyped → generic mention.
    if not out and a != b:
        emoji = {"session": "📝", "character": "🎒", "npcs": "💬",
                 "world": "🕒", "events": "📜"}.get(kind, "💾")
