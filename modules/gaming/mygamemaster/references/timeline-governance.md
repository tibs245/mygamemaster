...
`events.json` is the **temporal canon** of the campaign. Every event in it is an established, immutable fact. Nothing is speculative — we log only what has **actually occurred** in play.
...
```
campaigns/<name>/
├── events.json        ← The temporal canon — single file
├── tools/
...
2. **Add** the event to events.json
