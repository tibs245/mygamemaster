...
|---|---|---|---|---|---|
| **AGENCY-01** | Never write an action, gesture, posture, gaze, breath or movement of the PC — write only what the PC perceives. | BC-AGENCY | `code` — `agency_gate.py` `analyze` (violation `AGENCY-01`) + `redact`, called by `transform_llm_output.py` `enforce_agency`: the hook the runtime runs on the finished text of **every** turn, not a command the GM is asked to type. Downstream of inference nothing can be re-generated, so the remedy is a **cut** of the offending sentence and the correction leaves feed-forward. A detected violation is never delivered — that outcome depends on no model, network call, flag or budget; a crash of the analyser is not a verdict, the turn ships unchanged and is journalled `blind` in `.banquier/agency-gate.json` | `code` | S16 (cancelled), S23 (replayed), re-broken S26, S30 ×3 |
| **AGENCY-02** | Never put words in the PC's mouth: when the player gave substance without text, emit `[VERBATIM TO BE SUPPLIED BY THE PLAYER]`. | BC-AGENCY | `code` — same `enforce_agency` path; `analyze` (violation `AGENCY-02`) refuses PC speech and reads the placeholder as the prescribed correct form rather than a violation | `code` | S21, S23 |
| **AGENCY-03** | Narrate at most one PC action per turn, and only the direct execution of an action the player has just declared — cite the declaration. | BC-AGENCY | `code` — same `enforce_agency` path; `analyze` (violation `AGENCY-03`) counts PC actions and allows only the one whose verb appears in the player's declaration | `code` | S14 (3 stacked), S16 (8 stacked), S23 (4 chained) |
...
| **TURN-01** | One non-ordinary moment = one STOP = one logged action: never cross two non-ordinary moments in a single narration, and stay at the same STOP through dialogue sub-stops until the player gives an explicit decision. | BC-PACING | **`code`** — `hooks/turn_state.py` counts the moment MARKERS of a narration (`detect_moments`) and flags a second one on `transform_llm_output`; whether a moment is "non-ordinary" in the narrative sense stays `prompt` | **`code`** | S23 (5 moments, 1 STOP), S25 |
| **TURN-02** | Advance in time or space only after an explicit fast-forward signal — a discussion, a question or an intention is not one — and ask "what do you do?" before any ellipse longer than about an hour of game time. | BC-PACING | **`code`** — the signal is parsed by `pre_llm_call` from the player's verbatim message (unforgeable, un-skippable); a write advancing `current_day` without one is BLOCKED by `pre_tool_call` (the model must rework); the delivered text is flagged and fed forward by `transform_llm_output` | **`code`** | S17, S23 |
...
| **TURN-06** | When a fast-forward lands on a focal event, narrate that event in a few sentences and stop — no follow-on conversation, inventory or interiority. | BC-PACING | **`code`** — `hooks/turn_state.py` refuses two chained ellipses under one grant; "no follow-on conversation or interiority" stays `prompt` | **`code`** | S34 |
...
The other three — **AGENCY-01/02/03**, via `agency_gate.py` called from `transform_llm_output.py`
`enforce_agency` — are the first checks in this repo to guard a *narration in flight*. What earns
them the promotion is not the analyser but its **trigger point**: `transform_llm_output.py` is a
hook the runtime declares and runs on the finished text of every turn, so nothing is asked of the
model. The same analyser was reachable before only through `mj_checkpoint.py`, a command the prompt
*invites* the GM to run — and the corpus counted 8 agency violations in one hour under exactly that
arrangement. A check whose trigger is a line of prompt is a prompt rule with a runtime cost, which
is what rule 6 forbids promoting. `mj_checkpoint.py` still runs the same verdict one step earlier
and is strictly better when used — a draft submitted there can be *rewritten* rather than *cut* —
but it is an optimisation, not the guarantee, and the row is not written against it.
...
|---|---|---|
| `close_session.py` itself, the host of every *at-close* `code` row above | CLOSE-01, WORLD-03, WORLD-04 | it refuses correctly once it runs, but no hook, no CI step and no automation invokes it — `SKILL.md` §4 asks the GM to run it at close ("Run this script at close"), and a GM who declares the session closed from intention never triggers a single check |
...
| `llm_judge.py` (rubric AGENTIVITE, B2) and the `mj_checkpoint.py` gate | KNOW-01/02 — AGENCY-01/02/03 are no longer claimed here: they are enforced from `transform_llm_output.py` `enforce_agency`, which the runtime calls unconditionally | opt-in (`meta.hooks.judge.actif`, off by default), fail-open on any error, and the gate forces the turn through after `gate_max_tentatives` attempts. `mj_checkpoint.py` additionally only ever runs when the model chooses to run it — it is a useful earlier pass, never the guarantee |
