...
**Path:** `~/.hermes/mj-tonnerre/campagnes/<campaign-name>/monde.json`
...
```
python3 /opt/modules/gaming/mj-tonnerre/scripts/roll.py "<formula>" --dc <DC> --stat <Stat> --json
```
...
```
python3 /opt/modules/gaming/mj-tonnerre/scripts/roll.py "<formula>" --dc <DC> --stat <Stat> --quantique --json
```
...
4. **Determine the modifier** from the character sheet:
   - `personnages/<discord_id>.json` — stats, skills, bonuses
5. **⚠️ TIME & INCREASING COST of retries** — single source: umbrella `mj-tonnerre/SKILL.md → §10` (§10.1 every roll advances time; §10.2 retrying costs more). Apply it, do not copy it.
...
> **Format "Dice Roll" + rule "stat in parentheses": single source = umbrella `mj-tonnerre/SKILL.md` → `Formatting Conventions` → `### Dice Roll`.** Apply it as-is (inviolable format). Below: only what is SPECIFIC to this skill (dice detail, secret format).
...
- **Combat**: critical → damage + narrative effect if `critique_narratif` enabled in monde.json.
...
Before applying critical/fumble/thresholds, **always check** the system and house rules of the campaign in `monde.json`: `systeme` (`systeme.resolution`, `systeme.nat20`, `systeme.nat1`) and `regles` (house rules under `regles.*`). Possible keys depending on campaign:
