...
> The `history`, `session_id`, `notes_privees` fields are part of format v2. Old sheets may not have them: with each significant update, **add missing fields one by one**, without breaking existing fields. **Soft** migration — no forced rewrites.
...
| Field | Type | Usage |
|-------|------|-------|
| `history[]` | array of strings | Character's persistent facts/events — migrated from timeline and session actions. Each entry is a traceable fact (ex: "Repaired [a site] S1", "Met [an NPC] S4"). |
...
2. ✅ **Add** `history` if absent → populate from `world.json > global_state.timeline` and session logs
...
6. 🔄 **With each new session played**, enrich `history` with the session entry
...
| `goal` | `personal_notes.goals[]` | array operation |
...
| Key | Type | Meaning |
|-----|------|---------|
| `pacing` | string | Preferred pacing (e.g. "slow-burn investigation", "fast and reactive") |
| `tone_likes` | list | Tone the player enjoys (mystery, tension, lore…) |
| `tone_dislikes` | list | Tone to avoid (slapstick, bookkeeping…) |
| `combat_verbosity` | string | Combat verbosity (`concise` \| `vivid`) |
| `spotlight` | string | Spotlight preferences (shy, loves solo moments, never at others' expense…) |
| `content_boundaries` | list | Content boundaries (hard lines — always respected) |
| `enjoys_deception` | bool | Enjoys being deceived (a fair, foreshadowed twist) |
| `custom` | object | Any extra free-form keys the player asks to remember |

> Only these exact key names are recognized by `prefs.py` (`CLES_DOCUMENTEES`). Any other name is silently stored under `preferences.custom` and is **not** surfaced to the GM each turn — never invent a variant.
...
```bash
python3 /opt/modules/gaming/mygamemaster/scripts/prefs.py <campaign> <discord_id> set pacing "slow-burn investigation"
python3 /opt/modules/gaming/mygamemaster/scripts/prefs.py <campaign> <discord_id> set combat_verbosity concise
python3 /opt/modules/gaming/mygamemaster/scripts/prefs.py <campaign> <discord_id> set enjoys_deception true
python3 /opt/modules/gaming/mygamemaster/scripts/prefs.py <campaign> <discord_id> set tone_likes '["mystery","lore"]'
...
python3 /opt/modules/gaming/mygamemaster/scripts/prefs.py <campaign> <discord_id> get pacing    # one key
```

**Removing** a preference (player changes their mind):

```bash
python3 /opt/modules/gaming/mygamemaster/scripts/prefs.py <campaign> <discord_id> unset pacing
```
...
| Rule | Detail |
|-------|--------|
| **Max HP** | `hp_current` cannot exceed `hp_max` (except temporary effect → add to `health.conditions`). Message if exceeded: *"Your max HP is {max}. Cannot exceed this limit."* |
