...
APPLICATION:
  ...
  → Op.4: heure_courante += 15 min (early afternoon → early afternoon + 15min)
...
APPLICATION:
  → Op.4: heure_courante = "late afternoon D7"
...
| **§10 — Bug analyst mode** | Already covered by `mj-tonnerre-analyste` skill | Let `mj-tonnerre-analyste` handle bugs |
