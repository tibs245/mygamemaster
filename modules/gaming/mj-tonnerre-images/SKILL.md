...
When `mj-tonnerre-initiation` finishes creating a campaign, it must:
...
```bash
SCHEMA=/opt/modules/gaming/mj-tonnerre/scripts/carte_schema.py
GEN=/opt/modules/gaming/mj-tonnerre-images/scripts/generate_reviewed.py
...
```bash
SCRIPT=/opt/modules/gaming/mj-tonnerre-images/scripts/generate_reviewed.py
...
```
/opt/modules/gaming/mj-tonnerre-images/scripts/
...
```bash
REVIEW=/opt/modules/gaming/mj-tonnerre-images/scripts/review_image.py
python3 "$REVIEW" \
...
| Dependency | Role |
|-----------|------|
| `mj-tonnerre` (parent skill) | Campaign context, GM persona |
...
This skill is a sub-skill of `mj-tonnerre`. It is loaded automatically when triggers `!image`, `!portrait`, `!carte`, `!illustration` are detected.
