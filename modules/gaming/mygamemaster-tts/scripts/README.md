# Scripts — narrative voice (mygamemaster-tts)
...
| `MGM_TTS_FORMAT_MODEL` | model for format step (default `minimax/minimax-m3`) |
| `MGM_TTS_AMBIANCE_VOLUME` | volume of ambiance bed under voice (default `0.16`) |
| `MINIMAX_API_URL` | T2A endpoint (default `https://api.minimax.io/v1/t2a_v2`) |
| `MGM_TTS_MOCK=1` | mock synthesis offline (testing) |
| `MGM_TTS_FORMAT_MOCK=<json>` | format output injected offline (testing) |
