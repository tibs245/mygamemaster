#!/usr/bin/env python3
"""
test_tts.py — Offline tests for the mygamemaster-tts module (stdlib only, no network).

Everything is mocked: MGM_TTS_MOCK (fake synthesis) + MGM_TTS_FORMAT_MOCK (format output).
...
Run: python3 modules/gaming/mygamemaster-tts/tests/test_tts.py
"""
import json
import os
import subprocess
...
def main():
    total = 0
    print("=== mygamemaster-tts : offline tests ===\n")
    ...
    del os.environ["MGM_TTS_FORMAT_MOCK"]
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            os.environ.pop("MGM_TTS_FORMAT_MOCK", None)
    ...
    # Level 3: language-based via env MGM_LANGUAGE
    saved_lang = os.environ.pop("MGM_LANGUAGE", None)
    saved_langue = os.environ.pop("MGM_LANGUE", None)
    try:
        os.environ["MGM_LANGUAGE"] = "en"
        v, b = tts_render.resolve_voice_and_boost()
        check("MGM_LANGUAGE=en → Serene_Female", v == "Serene_Female", "got %s" % v)
        check("MGM_LANGUAGE=en → boost English", b == "English", "got %s" % b)

        # Level 2: world.json meta.audio.voice overrides language default
        with tempfile.TemporaryDirectory() as d:
        ...
        # Fail-open: missing world.json → no error, language env used
        with tempfile.TemporaryDirectory() as d:
            v, b = tts_render.resolve_voice_and_boost(campaign_dir=d)
            check("missing world.json → env language (Serene_Female)",
                  v == "Serene_Female", "got %s" % v)
    finally:
        if saved_lang is not None:
            os.environ["MGM_LANGUAGE"] = saved_lang
        else:
