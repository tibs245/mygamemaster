...
Before closing, apply the Banquier verification pipeline to all session actions.
...
**Available audit scripts (see also `mj-tonnerre-analyste` for 3 layers) :**
...
#### Step 1 — 🐞 Banquier Audit (`!analyse-bug`)

Calls `mj-tonnerre-analyste/SKILL.md` in **wrap-up audit mode**.
...
Calls `mj-tonnerre-game-report/SKILL.md`.
...
Calls `mj-tonnerre-write-history/SKILL.md`.
...
Calls `mj-tonnerre-images/SKILL.md` (use: session-end illustration).
...
0bis. **Living World Projection (B3) — if `meta.features.temporalite` != false AND `acteurs.json` present.** *(Everything ON by default: act on this only if explicitly `false`.)* Before ANY opening narration, project world to now :
   ```bash
   python3 /opt/modules/gaming/mj-tonnerre/scripts/world_tick.py pre . --apply
   ```
   ...
   - **FAIL-OPEN** : `world_tick.py` is fail-open and guards itself on `temporalite` (inert without `geo.json`/`acteurs.json`). If script fails or files missing → **ignore** and start normally. Living world is a bonus, never a requirement.
...
5.5. **⚠️ CROSS-CHECK CLOCK vs SESSION on Resumption** — Verify unplayed triggers from previous session :
   ...
   → References : `references/modules/factions.md` (Pitfall "CROSS-CHECK CLOCK vs SESSION") and
     hat checklist (`mj-tonnerre` §7, item `Cross-check horloge vs session`)
...
- **Sheet modification** → action type `modif_perso` (see `mj-tonnerre-personnage` skill)
...
> The multi-agent turn loop (GM / player / Banquier / NPC and Faction agents) is defined in hat `mj-tonnerre/SKILL.md §3.3`. Refer there — don't recoppy here.

Minimal reminder : Banquier applies its 3 transactional controls (SOURCE → TRANSFER → COHERENCE) regardless of action sender.
