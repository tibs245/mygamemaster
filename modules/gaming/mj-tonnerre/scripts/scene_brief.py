...
# worldlib.pj_ids(monde) (meta.pj_ids > meta.pj_id > env MJ_PJ_ID > []). A
...
def _axe_relationnel(campagne: Path, geo: dict, acteurs: dict, lieu_id: str,
    ...
    enjeux.sort(key=lambda x: (-(x["intensite"] or 0.0), x["acteur"]))
    ...
    mouvement.sort(key=lambda x: (x["T"] if isinstance(x["T"], int) else 0, x["acteur"]))
...
def _rendre_texte(brief: dict, acteurs: dict) -> str:
    ...
    if enjeux:
        ...
        morceaux = []
        for e in gardes:
            nom_e = _nom_acteur_court(e.get("acteur"), idx_act)
            typ = e.get("type", "?")
