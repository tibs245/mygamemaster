...
Proves the FAIL-OPEN, English-first contract:
  ...
  * resolve_lang cascade: env MJ_LANGUAGE > monde.meta.langue > 'en';
...
class TestResolveLang(unittest.TestCase):
    def setUp(self):
        self._saved = os.environ.pop("MJ_LANGUAGE", None)

    def tearDown(self):
        if self._saved is not None:
            os.environ["MJ_LANGUAGE"] = self._saved
        else:
