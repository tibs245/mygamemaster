...
(`mj-tonnerre-images`): the *code* fixes the composition, the *model* only needs
...
def main() -> int:
    ...
    if args.dump_semantique:
        ...
        modele = {"_doc": (
                      ...
                      "routes contournent — JAMAIS une barrière : le joueur peut toujours sortir "
                      "des routes (escalader, traverser un fleuve selon son courant…), et c'est "
                      "TOI qui arbitres la difficulté (outils : geo_query distance/chemin/"
                      "--vol-d-oiseau/croisement). `carte` = comportements selon le transport : "
                      "detours=false si on survole (jetpack, vaisseau) ; routes=false / "
                      "compass=false si ni voies ni nord (spatial). "
                      "Types : visible=false ⇒ jamais dessiné ; shape: rond|carre|triangle|"
                      "losange|etoile ; family: ville|biome|eau|repere|autre."),
                  "carte": _CARTE_CFG,
                  "types": eff}
