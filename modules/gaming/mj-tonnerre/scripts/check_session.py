...
def extraire_liste_pnj(pnj_data) -> list[dict]:
    ...
    {"pnj": [...]} (C1) or bare list [...] (C2)."""
    if isinstance(pnj_data, dict):
        return pnj_data.get("pnj", [])
...
def analyser(campagne: Path, num_session: int | None) -> dict:
    ...
    lieux_connus_norm = collecter_lieux_univers(monde)
    ...
    for f in factions:
        ...
        if not f.get("objectif_long_terme"):
    ...
    a_du_contenu = bool(session.get("actions") or session.get("lieux_visites")
                        or session.get("resume"))
    if a_du_contenu and not (session.get("heure_fin") or "").strip():
