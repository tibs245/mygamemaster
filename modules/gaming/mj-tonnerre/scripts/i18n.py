...
Design (fail-open, English-first):
  ...
  * The active language is resolved from a SINGLE source (`resolve_lang`):
    env override MJ_LANGUAGE first, then monde.json > meta.langue, then "en".
...
the language via monde.json > meta.langue (e.g. "de") or the MJ_LANGUAGE env var.
"""
...
def resolve_lang(monde=None) -> str:
    ...
    Cascade (most specific first):
        env MJ_LANGUAGE  >  monde.json > meta.langue  >  'en'.
    `monde` is the already-loaded monde.json dict (engine code always has it on
    hand); we never read the file here to keep this helper dependency-free.
    """
    env = os.environ.get("MJ_LANGUAGE")
    if env:
