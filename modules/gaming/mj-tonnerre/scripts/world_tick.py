...
is INJECTABLE (env var `MJ_AGENT_DECIDE_CMD`) and outside pure-code scope.
...
def appliquer_consequence(acteur: dict, intention: dict,
    ...
    seg = effets.get("trajectoire")
    if isinstance(seg, dict) and isinstance(seg.get("lieu"), str):
        traj = acteur.setdefault("trajectoire", [])
...
def _emettre_evenement(acteur: dict, intention: dict, T: int) -> dict:
    ...
    """
    action_slug = W.slug(intention.get("action", "") or intention.get("id", "evenement"))
    if not action_slug:
        action_slug = "evenement"
...
def _acteurs_dict(acteurs_idx: dict) -> dict:
    """Rebuilds a {'acteurs':[…]} container from an id→actor index."""
    return {"acteurs": list(acteurs_idx.values())} if isinstance(acteurs_idx, dict) else {"acteurs": []}
...
# Example: MJ_AGENT_DECIDE_CMD="hermes -p acteur-{slug}".  {slug} is substituted.
_ENV_AGENT_CMD = "MJ_AGENT_DECIDE_CMD"
...
def agent_decide(acteur: dict, brief: str, campagne: Path) -> dict:
    ...
    INJECTABLE implementation:
      ...
      * LLM implementation (gemma-4 via `hermes -p acteur-<slug>`): wired by
        the env var MJ_AGENT_DECIDE_CMD (the tick does NOT hard-code a network
    ...
    FEATURE GUARD "pnj_faction_vivants": if the axis is cut, we do NOT make actors
    "think" new plans via an external LLM agent — the MJ_AGENT_DECIDE_CMD command
    ...
    if cmd_modele and not W.features_campagne(Path(campagne)).get("pnj_faction_vivants", True):
        _log(f"ℹ agent_decide : axis « pnj_faction_vivants » cut → "
...
def _agent_decide_sousprocessus(acteur: dict, brief: str, cmd_modele: str) -> dict | None:
    ...
    slug = W.slug(aid.split(":", 1)[-1]) if isinstance(aid, str) else "acteur"
    try:
...
# ════════════════════════════════════════════════════════════════════════════
#  Feature flags — GUARD for the "temporalite" axis (living world ON/OFF)
# ════════════════════════════════════════════════════════════════════════════
#
# The meta.features.temporalite axis is the MAIN SWITCH of the living world engine
...
# (cascade meta.features > env MJ_FEATURE_* > True), so fail-open "all ON".

_MSG_TEMPORALITE_OFF = "temporality disabled (meta.features.temporalite=false)"
...
def _hook_actif(monde, nom: str) -> bool:
    ...
    are gated by temporalite, but can be cut independently).
    """
...
def _noop_pre(campagne: Path, motif: str = _MSG_TEMPORALITE_OFF,
              feature_temporalite: bool = False) -> dict:
    ...
    `motif` = reason for the no-op (temporalite axis OR meta.hooks.tick_pre toggle).
    `feature_temporalite` stays True when only the fine toggle cuts (axis still ON).
    """
    try:
    ...
    return {
        ...
        "briefing": motif,
        "ecritures": [],
        "feature_temporalite": feature_temporalite,
        "message": motif,
    }


def _noop_post(motif: str = _MSG_TEMPORALITE_OFF,
               feature_temporalite: bool = False) -> dict:
    """Coherent empty POST result when the tick is cut (no-op).

    `motif` = reason (temporalite axis OR meta.hooks.tick_post toggle).
    `feature_temporalite` stays True when only the fine toggle cuts (axis still ON).
    """
    return {
        "faits_joueur": [],
...
def pre(campagne: Path, t_session: int | None = None,
    ...
    if not W.features(monde).get("temporalite", True):
        _log("ℹ pre : " + _MSG_TEMPORALITE_OFF + " — no-op (no writes).")
        return _noop_pre(campagne)
    # Fine toggle: meta.hooks.tick_pre=false cuts PRE-processing (default true),
    # even with temporalite ON (cf. _lib.hooks_cfg / docs 10-features).
    if not _hook_actif(monde, "tick_pre"):
        _log("ℹ pre : " + _MSG_TICK_PRE_OFF + " — no-op (no writes).")
        return _noop_pre(campagne, motif=_MSG_TICK_PRE_OFF, feature_temporalite=True)
...
def post(campagne: Path, session: dict | str | None = None,
    ...
    # Main switch: "temporalite" axis cut → no-op (fail-open).
    if not W.features(monde).get("temporalite", True):
        _log("ℹ post : " + _MSG_TEMPORALITE_OFF + " — no-op (no writes).")
        return _noop_post()
    # Fine toggle: meta.hooks.tick_post=false cuts the closing reconciliation
    # (default true), even with temporalite ON (cf. _lib.hooks_cfg / docs 10-features).
    if not _hook_actif(monde, "tick_post"):
        _log("ℹ post : " + _MSG_TICK_POST_OFF + " — no-op (no writes).")
        return _noop_post(motif=_MSG_TICK_POST_OFF, feature_temporalite=True)
...
def extraire_faits_joueur(session: dict) -> list[dict]:
    ...
    for action in session.get("actions", []) or []:
        if isinstance(action, str):
            ...
            cible = action.get("cible") or action.get("acteur") or action.get("pnj")
            faits.append({
                "type": "action",
                "libelle": str(libelle),
...
def reconcilier_etat(acteur: dict, faits_joueur: list[dict], geo: dict) -> dict:
    ...
    plan_perturbe = False
    if not isinstance(acteur, dict):
        return {"acteur": None, "changements": changements, "plan_perturbe": False}
    ...
    return {"acteur": acteur.get("id"), "changements": changements,
...
def renouveler_plan(acteur: dict, faits_joueur: list[dict], campagne: Path) -> dict:
    ...
    for tentative in range(2):     # one retry max (feed-forward)
        ...
        if intention and valider_intention(intention):
            _inserer_intention(acteur, intention)
            return {"acteur": acteur.get("id"), "intention": intention, "refus": refus}
        refus.append(f"tentative #{tentative + 1} refusée (schéma/invariants).")
        brief += f"\n[Refus précédent : {refus[-1]} — corrige.]"

    return {"acteur": acteur.get("id"), "intention": None, "refus": refus}
...
def _appender_evenements(campagne: Path, evenements: list[dict]) -> int:
    ...
    existants = {e.get("id") for e in data["evenements"] if isinstance(e, dict)}
    n = 0
...
def _assembler_briefing(campagne: Path, T_de: int, T_a: int, ticks: list[dict],
    ...
    actifs = [t for t in ticks if t.get("lod") in ("tiede", "chaud") and t.get("evenements")]
    if actifs:
    ...
    # LOD summary.
    chauds = [t["acteur"] for t in ticks if t.get("lod") == "chaud"]
    tiedes_ids = [t["acteur"] for t in ticks if t.get("lod") == "tiede"]
...
def cmd_actor(args) -> int:
    ...
    if args.as_json:
        _sortir_json({"acteur": args.acteur_id, "operation": args.operation,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "model, export MJ_AGENT_DECIDE_CMD (e.g. 'hermes -p acteur-{slug}').\n"
        ),
    )
