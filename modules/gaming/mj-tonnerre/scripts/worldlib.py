...
def features(monde) -> dict:
    """Resolve the 5 feature flags. Cascade: meta.features.<axe> > env MJ_FEATURE_<AXE> > True.
...
def pj_id(monde) -> str | None:
    ...
    (`meta.pj_ids` > `meta.pj_id` > env `MJ_PJ_ID` > None). None =
...
def index_acteurs(acteurs: dict) -> dict[str, dict]:
    ...
    if not isinstance(acteurs, dict):
        return idx
    for a in acteurs.get("acteurs", []) or []:
...
def trajectoire_de(acteur: dict) -> list[dict]:
    """Return acteur['trajectoire'] or [].

    Backward-compat: if 'trajectoire' is absent but 'localisation_id' is present,
    builds a stay [{'lieu':loc,'de':0,'a':null}].
    """
    if not isinstance(acteur, dict):
        return []
    traj = acteur.get("trajectoire")
