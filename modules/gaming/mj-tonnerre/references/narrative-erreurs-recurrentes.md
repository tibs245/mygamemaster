...
**Principle:** Giving importance to the PC must never come at the expense of documented NPC-to-NPC relationships. **Check `pnj.json > faits_etablis` for relationships before assigning primacy to the PC.**
...
**Principle:** The timeline of Rubis in the March is **~6 game days**. Every time reference in NPC dialogue must be consistent with this timeline. **Read `monde.json > etat_global.chronologie` before writing a duration indication in dialogue.**
...
**Principle:** The journal was given by Firmin to Rubis the previous day ("I entrust it to you"). **Always verify object ownership in the PC or NPC inventory before describing it.** (See `personnages/<id>.json > inventaire`)
...
| New NPCs not filed | Esterlin + Karel missing from `pnj.json` |
...
| Timeline not enriched | Session 6 missing from `etat_global.chronologie` |
| Locations not added | Beech Path missing from `univers.regions[].lieux` |
