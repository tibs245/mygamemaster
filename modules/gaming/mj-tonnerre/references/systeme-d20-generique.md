...
- **Proficient** : fixed bonus to d20 (added to stat modifier) — the exact value is defined by `world.json > system.competences`
...
HP and combat style are defined per campaign in `world.json > system.sante` and `system.combat`. The generic framework: health is measured in Hit Points; combat leans toward narration and choice rather than calculations, unless the campaign declares a higher crunch level (`system.crunch`).
