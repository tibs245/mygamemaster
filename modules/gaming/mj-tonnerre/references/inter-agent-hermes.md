...
- The `mj-tonnerre-pnj` skill (protocol: golden rule, 🎭/🎯/❓/🔒 format, guardrails)
...
Both scripts live in `/opt/modules/gaming/mj-tonnerre/scripts/`.
...
- Skill `mj-tonnerre-pnj` loaded via `-s` and applied correctly
...
**Golden Rule:** Each NPC profile loads the `mj-tonnerre-pnj` skill.
