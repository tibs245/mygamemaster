...
def main():
    ...
    if not sb:
        print("No metrics yet (scoreboard.json missing)."); return 0

    print("📊 Scoreboard per model — campaign %s\n" % camp.name)
    hdr = "%-32s %6s %7s %7s %9s %9s %6s" % (
        "model", "turns", "clean", "%clean", "banker", "conduct", "forced")
    ...
    for model, m in sorted(sb.items(), key=lambda kv: -int(kv[1].get("tours", 0))):
        ...
        if par_regle:
            ...
            print("    top rules: " + ", ".join("%s×%d" % (k, v) for k, v in top))
    return 0
