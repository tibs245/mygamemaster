...
**Absolute visual consistency** — each element has a fixed format that players recognize instantly. **These templates are the single source of truth** : sub-skills (`mj-tonnerre-session`, `-personnage`) *refer to them* without recopying the formatting.
...
The `/opt/modules/gaming/mj-tonnerre/scripts/` folder provides machine guards (see its `README.md`). All take campaign path as argument, clean exit codes, stdlib only:
...
*(Compartmentalization is detailed in §9 "The GM Keeps Secrets". The `mj-tonnerre-personnage` sub-skill refers to it without recopying.)*
...
> `echo "<your draft>" | python3 /opt/modules/gaming/mj-tonnerre/hooks/mj_checkpoint.py --declared "<player action>"`
...
(SOURCE / TRANSFER / COHERENCE)** to recent scene actions — canonical formulation and detail in `mj-tonnerre-intendant/SKILL.md §2`.
...
> **Level 2 NPC/Faction agents are an approved feature, toggleable per campaign.** Their design lives in skills `mj-tonnerre-pnj` (NPC-agent) and
> `mj-tonnerre-faction` (Faction-agent), and reference `references/inter-agent-hermes.md`.
> The Steward (`mj-tonnerre-intendant`) remains the transactional verification standard,
> regardless of action source.

**The Steward applies the 3 transactional controls** (canonical formulation:
`mj-tonnerre-intendant/SKILL.md §2`) regardless of action source — player, NPC played by GM, or faction. The Steward verifies the transaction, not the source.
...
`mj-tonnerre-intendant/SKILL.md §2`). **NEVER send a response if any of the 3 controls fail** —
...
3. 🧮 STEWARD VERIFIES — the 3 controls SOURCE / TRANSFER / COHERENCE
   (see `mj-tonnerre-intendant §2`). → ✅ VALID / ❌ REFUSE (reason + file ref).
...
| `tts` | Disables **narrative voice** (auto-voice of narration **and** `!raconte` command, see `mj-tonnerre-tts`) — written text unchanged. *Fine cut: keep `!raconte` but cut auto-voice → `meta.hooks.tts_auto=false`.* |
...
| `!init` | Launches onboarding questionnaire (loads `mj-tonnerre-initiation`) |
...
| Command | Effect |
|---------|--------|
| `!game-report` | Generate game report (loads `mj-tonnerre-game-report`) |
| `!write-history` | Write campaign narrative history (loads `mj-tonnerre-write-history`) |
| `!image <description>` | Generate illustration (loads `mj-tonnerre-images`) |
...
> **Admin bypass note :** an author listed in `meta.admins`/`MJ_ADMIN_IDS` is always in bypass (no scrub, no "Persisted" block), **but** LLM judge still runs on their turns — only explicit `⏸️`/`▶️` suspends it. To truly pause game (even as admin), use `⏸️`, not just admin status.
...
| `!bug <description>` | Reports technical bug → report (loads `mj-tonnerre-bug-report`) |
| `!mj-log` | Shows latest MJ-INTENTION-LOG.md entries |
| `!mj-log ajoute <text>` | Adds entry to MJ-INTENTION-LOG.md (GM use only) |
| `!verbosite <level>` | Changes Steward verbosity level (TRACE/DEBUG/INFO/WARN/ERROR) — see `mj-tonnerre-intendant` §11 |
...
| `!audit-presession` | Pre-session coherence audit (loads `mj-tonnerre-analyste` mode C) |
| `!features` / `!feature` | Shows effective state of 6 feature flags (`meta.features`) — `feature_toggle.py --list` |
| `!feature <axis> on\|off` | **Admin** (`meta.admins`/`MJ_ADMIN_IDS`) : toggles axis **hot** (effect next turn, no redeployment) — relays script message, warning included for structural axis |
...
| `mj-tonnerre-analyste` | `!analyse-bug`, `!audit-presession` (mode C) |
| `mj-tonnerre-game-report` | `!game-report` |
| `mj-tonnerre-write-history` | `!write-history` |
| `mj-tonnerre-bug-report` | `!bug` |
| `mj-tonnerre-intendant` | `!verbosite`, `!collecte stats`, `!collecte dernieres` |
