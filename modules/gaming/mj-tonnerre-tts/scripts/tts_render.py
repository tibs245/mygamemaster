...
Chains, in a single call, the full quality pipeline — without loading the GM model:
  ...
  2. tts_generate.synthesize      → MP3 voice (Minimax speech-2.8-turbo,
     ...
     call, never a tag in the text. Disabled via MJ_TTS_SEGMENT=0; fallback to mono-call
...
Test modes: MJ_TTS_MOCK=1 (mock synthesis) + MJ_TTS_FORMAT_MOCK=<json> (format).
"""
import argparse
import json
import os
...
AMBIANCE_VOLUME = float(os.environ.get("MJ_TTS_AMBIANCE_VOLUME", "0.16"))
...
def _segment_enabled():
    """Multi-emotion voice by segments. ON by default; disabled via MJ_TTS_SEGMENT=0."""
    return os.environ.get("MJ_TTS_SEGMENT", "1").strip().lower() not in (
        "0", "false", "off", "no", "non")
...
def write_sidecar(out_path, report):
    ...
    meta["generator"] = "mj-tonnerre-tts/render"
...
def main():
    ...
    if not os.environ.get("MINIMAX_API_KEY") and not os.environ.get("MJ_TTS_MOCK"):
