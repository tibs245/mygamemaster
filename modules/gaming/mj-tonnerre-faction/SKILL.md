...
> ✅ **Approved — activatable/deactivatable per campaign.** Level 2 agent (embodies ONE faction with limited awareness). Provisioning: one-container-per-campaign model (Hermes' "profiles" approach is obsolete, cf. specs). Action validation: the Steward (`mj-tonnerre-intendant`).
...
> **Key difference from the NPC Agent** (`mj-tonnerre-pnj`): an NPC Agent plays **one person**; you play **a group**. You reason at the faction scale (decisions, resources, deadlines, territory), not at the individual scale. You **compress**: you do not simulate each of your 3, 30, or 300 members — you decide on behalf of the collective and let the GM populate the details.

Design ref: `archive_hermes/audit/06-niveau2-factions.md` (archived; collective intelligence = compression, §1 and §9). Data model: `/opt/modules/gaming/mj-tonnerre/references/modules/factions.md`.
...
Full detail: `/opt/modules/gaming/mj-tonnerre/references/modules/factions.md` § "Golden rules".
...
- **Parent skill**: `mj-tonnerre` (umbrella — multi-agent turn loop, factions module).
- **Twin skill**: `mj-tonnerre-pnj` (same protocol, individual scale).
- **Validation**: `mj-tonnerre-intendant` (the Steward — "action-by-action validation" mode).
- **Data model**: `/opt/modules/gaming/mj-tonnerre/references/modules/factions.md`.
